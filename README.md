...
**Leaver workflow:** Built out the final stage of the Joiner-Mover-Leaver lifecycle — the Leaver process — which follows this flow:
...
**Rehire / Reactivation workflow:** An employee's status is changed back to Active directly in the HR source (SimplifyHR) — testing the rehire scenario, which is common in real enterprise environments.
...
**A note on automation's blind spot:** Automation solves the problem of manual processes not keeping pace with scale, but it introduces a different risk: because the process runs in the background, nobody sees it happening day to day, and it's easy to assume it's working correctly simply because nobody's had to think about it. A Mover event that silently adds new group access without removing the old is precisely the kind of thing that can go unnoticed for a long time in an automated system — someone accumulates permissions from two or three past roles they no longer need, and because no human is manually reviewing each change, it's rarely caught until an access review or an audit surfaces it.
...
**Non-Human Identity (NHI) — Discovery and Governance Gap**

While building the Joiner-Mover-Leaver lifecycle for human employees, I identified a Non-Human Identity already present in the lab environment: the **Bind DN** used by midPoint to authenticate to OpenLDAP (`cn=Directory Manager`). This account has full write access to the entire LDAP directory — the same access midPoint uses to create accounts, modify attributes, and move users to `ou=inactive` on termination.

Unlike every human identity in this lab, this account was never provisioned through HR, has no manager, no department, no start date, and no leaver trigger. It exists permanently because the integration between midPoint and LDAP requires it to function — not because a business event created it.

**Why this matters:** the entire JML lifecycle built in this project depends on HR being the single source of truth, with every change flowing through midPoint's governance and audit trail. If the Directory Manager credentials were compromised, an attacker could bypass midPoint and the governance layer entirely — connecting directly to LDAP to create accounts, reactivate disabled ones, or grant themselves group membership, with no HR record, no reconciliation task, and no audit trail showing why any of it happened. The same risk applies directly to real enterprise environments — a compromised service account credential for Active Directory or Entra ID would let someone create or reactivate accounts at the target directly, bypassing whatever IGA or governance layer sits on top. The governance built on top is only as strong as the credential underneath it.

**Why human governance tools don't map onto NHIs:** standard identity governance controls — access reviews, entitlement certification — are built around a human context: a manager confirming someone still needs access tied to their current role. A service account like Directory Manager has no manager and no changing role to review against; it permanently needs the same access to keep functioning. Applying a human-style access review to it doesn't really work. NHIs instead require different governance mechanisms: credential rotation on a schedule, least-privilege scoping (splitting one powerful account into narrower ones), usage/anomaly monitoring, maintaining a service account inventory, and assigning a human or team as accountable owner even though nobody logs in as the account day to day.

**Hands-on test — creating an NHI outside governance:** to confirm this risk directly rather than just reason about it, I created a second service account (`svc-backup`) manually inside LDAP via phpLDAPadmin, simulating what a developer or sysadmin might do directly against the target system, bypassing the IGA platform entirely. Checking midPoint's Users → All Users afterward confirmed `svc-backup` did not appear anywhere in midPoint — proof that an account created directly at the target system is completely invisible to the governance platform sitting above it.

**Testing whether this gap could be closed:** I then attempted to run a reconciliation task against the OpenLDAP resource, to see whether midPoint could detect `svc-backup` after the fact by scanning the target directory. This failed with a fatal error (no object type definition for the Account type on this resource), indicating this lab's OpenLDAP resource was only configured to support one-directional provisioning (midPoint → LDAP), not reconciliation in the reverse direction (LDAP → midPoint).

**Conclusion:** this confirms a one-directional blind spot in identity governance: midPoint pushes changes from HR out to LDAP, but does not automatically watch LDAP for unauthorized changes made in the other direction. A real enterprise needs a separate, deliberately-built discovery or audit control — reconciling the target system back against the governance platform's records — specifically to catch accounts like `svc-backup`: created directly at the target, with no HR record, no audit trail, and no natural trigger that would otherwise reveal their existence.
